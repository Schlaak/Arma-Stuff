...
creates illusion of nearby gunfight
*/
//--------------------------------------------------------------------------------------------
...
_handle = [true, 3000] spawn { //[debug mode, duration]
	...
	_dist = 400;
	_center = center; //object with varname = center
	_headPos = _center;
	if (_debug) then {
		hint "starting audio";
	};
	...
	while {time < _end} do {
		...
		for "_i" from 0 to 5 do { //spawn up to 5 salvos of fast fire (MG)
			...
			//params["_sound","_pos","_shots","_delay","_volume"];
			[_sound,[0,0,0],5,random 10,0.5 + random 0.5,_center,_dist] remoteExec ["IRN_remoteSound",0, true];
		};
